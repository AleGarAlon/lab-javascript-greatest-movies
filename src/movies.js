...
function turnHoursToMinutes(moviesArray) {
    const moviesArrayMinutes = moviesArray.splice(0, moviesArray.length);

    const numbers = /\d+/g;

    moviesArrayMinutes.forEach(function (element){
        element.duration = ((element.duration).match(numbers)[0]) * 60 + (element.duration).match(numbers)[1]
    }
